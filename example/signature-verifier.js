 ...
 * a. Signing a message by an Cypherium user
...
function initializeCypheriumConnection(){
   if(ethWeb3!=null && ethWeb3.isConnected()==true)  {
    return true;
  }
  
  ethWeb3 = new Web3c(new Web3c.providers.HttpProvider(ethURL));
  
  if(ethWeb3.isConnected()==true){
      if(defaultAc==''){
        defaultAc=ethWeb3.cph.accounts[1];
      }
      return true;
  }
...
function initializeContract(){
    initializeCypheriumConnection();
    if(ethWeb3.isConnected()==false){
        return;
    }  
    var abi = JSON.parse(strAbi);
    var contract = ethWeb3.cph.contract(abi);
...
function signMessage(message){
    ...
    const sig = ethWeb3.cph.sign(defaultAc, '0x' + msg.toString('hex'));
...
function verifySignedByAc(message, sig){
    ...
    const prefix = new Buffer("\x19Cypherium Signed Message:\n");
    const msg = new Buffer(message);
...
function execute(){
    ...
    console.log("**********************************************************************");
    console.log("1. Deploy the following contract in your cypherium environment");
    console.log(signatureContractCodeReadable);
    console.log("2. Set the following parameters (i.e. at the end of the code)");
    console.log("\ta. Cypherium URL");
    console.log("\tb. Cypherium Account Address");
    console.log("\tc. Cypherium Account Passphrase");
    console.log("\td. Signature Contract Address");
    console.log("\te. Message for signing");
    console.log("**********************************************************************");

    if(ethURL==''){
        console.log("Error: Cypherium URL is not specified");
        return;
    }
    if(defaultAc==''){
    ...
    console.log("\ta. Cypherium URL                  :",ethURL);
    console.log("\tb. Cypherium Account Address      :",defaultAc);
    console.log("\tc. Cypherium Account Passphrase   :",defaultAcPWD);
...
// Value 1- Please provide the cypherium account address which you want to use to perform the operation
...
setCypheriumURL('http://localhost:8545');

// Value 5- If required please update with a Cypherium URL
